...
`,
  ...
  concat: {
    name: 'Concat',
    code: `
const { of, concat, interval, zip } = Rx;
const { map } = RxOperators;

// concat is like adding streams
concat(
  zip(of(1, 2, 3), interval(1000)).pipe(map(([x, t]) => x)),
  zip(of(4, 5, 6), interval(1000)).pipe(map(([x, t]) => x)),
  zip(of(7, 8, 9), interval(1000)).pipe(map(([x, t]) => x)),
);
...
`,
  ...
  concatAll: {
    name: 'Concat all',
    code: `// Collect observables and subscribe to next when previous completes.
const { of, interval } = Rx;
const { map, concatAll, take } = RxOperators;

//emit 0, 1, 2, ... every 1 second
interval(1000).pipe(
...
`,
  ...
  combineAll: {
    name: 'Combine all',
    code: `
const { interval, of, zip } = Rx;
const { take, map, combineAll } = RxOperators;

const numbers$ = interval(1000);

const chars$ = zip(
  ['a', 'b', 'c', 'd', 'e'], interval(2000)
).pipe(map(([c, n]) => c));

// combineAll is like full-join for streams
of(numbers$, chars$).pipe(combineAll());
`,
    timeWindow: 10000
  },
  merge: {
    name: 'Merge',
    code: `// merge is like union for streams
const { interval, merge, zip } = Rx;
const { mapTo, map } = RxOperators;

const numbers$ = interval(1100);

const letters$ = zip(
  ['a', 'b', 'c', 'd', 'e'], interval(1700)
).pipe(map(([letter, number]) => letter));
